...
$config['agregar']="yes";
$config['delete']="yes";
$config['form_edit']="forms/secciones.inc.php";
$config['listar_condicion']="tipo = $config[tipo]";
$config['em_condicion']="id_menu=$id";
$config['eliminar_control']="";
$config['modificar_control']=array('e_menues'=>"titulo='$titulo[1]'  AND id_menu!='$id'");
$config['alta_control']=array('e_menues'=>"titulo='$titulo[1]'");
...
switch($op){
	...
	case 'ce':{
      ...
      if ($ok = eliminar($config, $dbm)){
			$query="DELETE FROM e_menu_adic WHERE menu_id=$id";
			$dbm->Execute($query);
		} else {
			$dbm->FailTrans();
		}
	...
	case 'o':{
		...
		$config['op']="cm";
		include('include/templates/form_header.inc.php');
		$config['include_form']="forms/pies_ord.inc.php";
	...
	case 'cmo':{
		$dbo = connect();
		$e_secc=split(",",$smenuord_lst);
		$i=0;
		foreach($e_secc as $k){
  			if($k!=""){
				$secco=split("~",$k);
				$i++;
				$seccordq = "UPDATE e_menues SET posicion=$i WHERE id_menu=$secco[1]";
				$ordresult = $dbo->Execute($seccordq);
			}
		}
		dolog("Cambio de ordenamiento de secciones en el pie");
		$config['condicion']=$config['listar_condicion'];
		listar($config);
		break;
	}
