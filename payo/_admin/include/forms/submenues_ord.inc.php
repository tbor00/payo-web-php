...
$db = connect();
$db->debug = SDEBUG;
$menu_query = "SELECT id_menu,titulo FROM e_menues WHERE $config[condicion] AND tipo=2 AND lenguaje_id=1 ORDER BY posicion";
if ($result = $db->Execute($menu_query )) {
	while( !$result->EOF ){
		echo "<OPTION VALUE=\"p0~".$result->fields[id_menu]."\">".$result->fields[titulo]."</OPTION>\n";
		$submenu_query = "SELECT id_menu,titulo FROM e_menues WHERE $config[condicion] AND tipo=3 AND lenguaje_id=1 AND menu_id=".$result->fields[id_menu]." ORDER BY posicion";
