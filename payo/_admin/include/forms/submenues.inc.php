...
echo "AddCampo(\"titulo[1]\", 2, \"" . convert_str("el campo Título en Español",$default->encode)."\", \"text\",'');\n";
echo "AddCampo(\"menu_id[1]\", 2, \"" . convert_str("para Pertenece a:",$default->encode)."\", \"select\",'document.form_menues.elements[\"activo[1]\"].checked');\n";
echo "AddCampo(\"titulo[2]\", 2, \"" . convert_str("el campo Título en Inglés",$default->encode)."\", \"text\",'');\n";
echo "AddCampo(\"menu_id[2]\", 2, \"" . convert_str("para Pertenece a:",$default->encode)."\", \"select\",'document.form_menues.elements[\"activo[2]\"].checked');\n";
echo "</SCRIPT>\n";
?>
...
?>
<TR>
<TD><STRONG>Pertenece a:&nbsp;</STRONG></TD>
<TD><SELECT NAME="menu_id[1]" CLASS="select" SIZE="1">
<?php
$menudb = connect();
$menudb->debug = SDEBUG;
$menuq = "SELECT id_menu, titulo, posicion FROM e_menues WHERE tipo=({$config['tipo']} - 1) AND lenguaje_id = 1 ORDER BY posicion";
if ($menur = $menudb->Execute($menuq)){
	echo "<OPTION VALUE=\"0\">---- Selecione un $padre ----</OPTION>\n";
	while( !$menur->EOF ){
...
<TD COLSPAN="2"><INPUT NAME='bgcolor[1]' CLASS="boxes" VALUE="<?php echo $formdata[1]->fields['bgcolor']; ?>" TYPE='TEXT' SIZE='15' MAXLENGTH='15'></TD> 
</TR>
<TR>
...
<TD><TEXTAREA NAME="texto_up[1]" CLASS="editboxes" ROWS="9" COLS="75"><?php echo htmlspecialchars($formdata[1]->fields['texto_up']) ?></TEXTAREA></TD> 
...
<TD><TEXTAREA NAME="texto_down[1]" CLASS="editboxes" ROWS="9" COLS="75"><?php echo htmlspecialchars($formdata[1]->fields['texto_down']) ?></TEXTAREA></TD> 
...
<TR>
<TD><STRONG>Pertenece a:&nbsp;</STRONG></TD>
<TD><SELECT NAME="menu_id[2]" CLASS="select" SIZE="1">
<?php
$menudb = connect();
$menudb->debug = SDEBUG;
$menuq = "SELECT id_menu, titulo, posicion FROM e_menues WHERE tipo=({$config['tipo']} - 1) AND lenguaje_id = 2 ORDER BY posicion";
if ($menur = $menudb->Execute($menuq)){
	echo "<OPTION VALUE=\"0\">---- Selecione un $padre ----</OPTION>\n";
	while( !$menur->EOF ){
		echo "<OPTION VALUE='".$menur->fields[id_menu]."'";
...
<TR> 
<TD><STRONG>Color de Fondo:</STRONG></TD> 
<TD COLSPAN="2"><INPUT NAME='bgcolor[2]' CLASS="boxes" VALUE="<?php echo $formdata[2]->fields['bgcolor']; ?>" TYPE='TEXT' SIZE='15' MAXLENGTH='15'></TD> 
</TR>
<TR>
<TD><STRONG>Ejecutar:</STRONG></TD>
...
<?php
echo "<INPUT TYPE=\"HIDDEN\" NAME=\"tiposel\" VALUE=\"\">\n";
