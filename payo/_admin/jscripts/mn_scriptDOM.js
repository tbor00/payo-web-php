...
mn_IE5M = mn_IE && mn_Mac;
mn_IE5W = mn_IE && !mn_Mac;
mn_NS6 = (navigator.vendor == ("Netscape6") || navigator.product == ("Gecko"));
mn_IEpos = mn_IE || (mn_NS6 && parseInt(navigator.productSub)>=20010710);
mn_IEDTD = (mn_IE && document.doctype) ? document.doctype.name.indexOf(".dtd")!=-1 : false;
mn_IEnoDTD = mn_IE && !mn_IEDTD;
...
function mn_f_MakeTrees(){
	...
	for(var t=0; t<mn_a_TreesToBuild.length; t++) {
		...
		if(mn_CurrentTree.TopIsPermanent){
			...
			if(mn_IE5M) setTimeout(mn_CurrentTree.treeParent.id + ".fixSize(true)",10);	
			else mn_CurrentTree.treeParent.style.visibility = "visible";
		}
    }
...
function mn_f_SetItemProperties(itemidsuffix) {
...
function mn_f_MakeElement(menuid) {
	...
	with(MenuObject){
		...
		with(style) {
			...
			left = "-500px";
			top = "0px";
			width = (mn_IEnoDTD ? mn_CurrentTree.MenuWidth : mn_CurrentTree.ItemWidth) + "px";
		}
	}
...
function mn_f_MakeMenu(menucount) {
	...
	NewMenu = document.getElementById(mn_MenuIDPrefix + menucount);
	if(!NewMenu){
		...
		if(mn_CurrentMenu) {
			...
			mn_CurrentTree.treeParent = mn_CurrentTree.startChild = NewMenu;
		}
		...
		if(mn_IE) mn_CurrentMenu.onselectstart = mn_f_CancelSelect;
	...
	while (mn_CurrentMenu.itemCount < mn_CurrentMenu.maxItems) {
		...
		mn_CurrentMenu.itemElement = document.getElementById(mn_ItemIDPrefix + menucount + "_" + mn_CurrentMenu.itemCount);
		if(!mn_CurrentMenu.itemElement){
			if(mn_StatusDisplayBuild) status = "Creating Hierarchical Menus: " + menucount + " / " + mn_CurrentMenu.itemCount;
			mn_CurrentMenu.itemElement = mn_f_MakeItemElement(menucount);
		}
	...
	if(!mn_IE5M)mn_CurrentMenu.fixSize();
...
function mn_f_MakeItemElement(menucount) {
	var ItemElement = document.createElement("DIV");
	ItemElement.id = mn_ItemIDPrefix + menucount + "_" + mn_CurrentMenu.itemCount;

	ItemElement.style.position = "absolute";
	ItemElement.style.visibility = "inherit";
	mn_CurrentMenu.appendChild(ItemElement);
	ItemElement.menu = mn_CurrentMenu;
	ItemElement.setItemProperties = mn_f_SetItemProperties;
	ItemElement.setItemProperties(menucount + "_" + mn_CurrentMenu.itemCount);
	ItemElement.siblingBelow = ItemElement.previousSibling;
	if(ItemElement.linkText && !ItemElement.ClickStart) {
		ItemElement.onclick = mn_f_LinkIt;
		if(mn_ShowLinkCursor)ItemElement.style.cursor = mn_NS6 ? "pointer" : "hand";
	}
	var FullPadding  = (mn_ItemPadding*2) + mn_ImageSize + mn_ImageHorizSpace;

	ItemElement.hasImage = (ItemElement.hasMore && mn_CurrentMenu.showImage);
    if(ItemElement.hasImage) {
		var ImageElement = document.createElement("IMG");
		ItemElement.imageSrc = mn_CurrentTree.RightToLeft ? mn_ImageSrcLeft : mn_ImageSrc;
		ItemElement.hasImageRollover = ((!mn_CurrentTree.RightToLeft && mn_ImageSrcOver) || (mn_CurrentTree.RightToLeft && mn_ImageSrcLeftOver));
		if(ItemElement.hasImageRollover) {
			ItemElement.imageSrcOver = mn_CurrentTree.RightToLeft ? mn_ImageSrcLeftOver : mn_ImageSrcOver;
		...
		with(ImageElement){
			...
			hspace = (!mn_CurrentTree.RightToLeft && mn_IE5W) ? mn_ItemPadding : 0;
			vspace = 0;
			width = mn_ImageSize;
			with(style) {
				position = "absolute";
				top = (mn_ItemPadding + mn_ImageVertSpace) + "px";

				if(mn_CurrentTree.RightToLeft) {
					left = (mn_ItemPadding + mn_ImageHorizSpace) + "px";
				}
			}
		}
...
function mn_f_SetItemStyle() {
	with(this.style){
		...
		if((this.menu.showImage && (!this.menu.IsVariableWidth || (this.menu.IsVariableWidth && this.tree.RightToLeft && !this.menu.isHorizontal))) || (this.menu.IsVariableWidth && this.imgLyr)) {
			...
			var FullPadding  = (mn_ItemPadding*2) + mn_ImageSize + mn_ImageHorizSpace;
			if (mn_CurrentTree.RightToLeft) paddingLeft = FullPadding + "px";
		...
		if(this.menu.isHorizontal){
			...
			if(!this.menu.IsVariableWidth) {
				if(mn_IEnoDTD){
					if(this.isLastItem) width = (mn_CurrentTree.MenuWidth - mn_BorderWidth - mn_SeparatorSize) + "px";
					else width = (mn_CurrentTree.MenuWidth - mn_BorderWidth) + "px";
					left = (this.index * (mn_CurrentTree.MenuWidth - mn_BorderWidth)) + "px";
				}
				else {
					width = (mn_CurrentTree.MenuWidth - mn_BorderWidth - parseInt(paddingLeft) - parseInt(paddingRight) - mn_SeparatorSize) + "px";
					left = ((this.index * parseInt(width)) + ((mn_SeparatorSize * this.index)))  + ((parseInt(paddingLeft) + parseInt(paddingRight)) * this.index) + "px";
				}
				var LeftAndWidth = parseInt(left) + parseInt(width);
				this.menu.style.width = LeftAndWidth + (mn_IEnoDTD ? (mn_BorderWidth * 2) : (parseInt(paddingLeft) + parseInt(paddingRight))) + "px"
			}
		}
		else {
			left = "0px";
			if(!this.menu.IsVariableWidth) {
				if(mn_IEnoDTD) width = mn_CurrentTree.ItemWidth + "px";
				else width = (mn_CurrentTree.ItemWidth - (parseInt(paddingLeft) + parseInt(paddingRight))) + "px";
			}
	...
	if(this.isHorizontal) {
		if(this.IsVariableWidth) {
		    for(i=0; i<this.childNodes.length; i++) {
				...
				TempItem.realWidth = mn_IE ? TempItem.scrollWidth : TempItem.offsetWidth;
				if(mn_IE5M) TempItem.realWidth += (parseInt(TempItem.style.paddingLeft) + parseInt(TempItem.style.paddingRight))
				if(mn_IEnoDTD){
					if(TempItem.isLastItem) TempItem.style.width = (TempItem.realWidth) + "px";
					else TempItem.style.width = (TempItem.realWidth + mn_SeparatorSize) + "px";
					...
					if(TempItem.hasImage && !mn_CurrentTree.RightToLeft) {
						TempItem.realWidth += mn_ItemPadding;
					}
					TempItem.realWidth -= (parseInt(TempItem.style.paddingLeft) + parseInt(TempItem.style.paddingRight))
 					if(!mn_IE5M && !TempItem.isLastItem)  TempItem.realWidth -= mn_SeparatorSize;
				...
				if(TempItem.isLastItem) {
					...
					this.style.width = LeftAndWidth + (mn_IEnoDTD ? (mn_BorderWidth * 2) : (parseInt(TempItem.style.paddingLeft) + parseInt(TempItem.style.paddingRight))) + "px";
				}
			}
		}
	    ...
	    for(i=0; i<this.childNodes.length; i++) {
	        var TempItem = this.childNodes[i];
		    if(TempItem.index) {
				var SiblingHeight = TempItem.siblingBelow.offsetHeight - (mn_IEnoDTD ? 0 : (mn_ItemPadding * 2));
				MaxItemHeight = Math.max(MaxItemHeight,SiblingHeight);
			}
	       	else {
				MaxItemHeight = TempItem.offsetHeight - (mn_IEnoDTD ? 0 : (mn_ItemPadding * 2));
			}
		}
	    for(i=0; i<this.childNodes.length; i++) {
	        var TempItem = this.childNodes[i];
			...
			if(TempItem.imgLyr) {
				if(this.tree.RightToLeft){
					TempItem.imgLyr.style.left = (mn_ItemPadding + mn_ImageHorizSpace) + "px";
				}
				else {
					TempItem.imgLyr.style.left = (TempItem.offsetWidth - ((TempItem.isLastItem ? 0 : mn_SeparatorSize) + (mn_IEnoDTD ? mn_IE5M ? mn_ItemPadding : mn_ItemPadding * 2 : mn_ItemPadding) + mn_ImageHorizSpace + mn_ImageSize)) + "px";
				}
			}
		}

		this.style.height = MaxItemHeight + (mn_IEnoDTD ? mn_BorderWidth * 2 : (mn_ItemPadding * 2)) + "px";
	}
	else {
		if(this.IsVariableWidth) {
			var MaxItemWidth = 0;
	    	...
	    	for(i=0; i<this.childNodes.length; i++) {
				this.childNodes[i].style.width = (TempWidest.realWidth + (mn_IEnoDTD ? 0 : (0))) + "px";
				if(!mn_IEnoDTD) {
			...
			this.style.width = (this.childNodes[0].offsetWidth +  (mn_IEnoDTD ? mn_BorderWidth * 2 : 0)) + "px";
		}
	    ...
	    for(i=0; i<this.childNodes.length; i++) {
			...
			if(TempItem.imgLyr) {
				if(this.tree.RightToLeft){
					...
					TempItem.imgLyr.style.left = (TempItem.offsetWidth - ((mn_IEnoDTD ? mn_IE5M ? mn_ItemPadding : mn_ItemPadding*2 :  mn_ItemPadding) + mn_ImageHorizSpace + mn_ImageSize)) + "px";
				}
			}
		}
		this.style.height = parseInt(TempItem.style.top) + (mn_IE5W ? TempItem.scrollHeight : TempItem.offsetHeight) + (mn_IEnoDTD ? (mn_BorderWidth * 2) : 0) + "px";
	}
...
function mn_f_PopUp(menuname,e){
	...
	if (mn_CurrentMenu.tree.ClickStart) {
		var ClickElement = (mn_IE) ? e.srcElement : e.target;
		if(mn_NS6) {
...
function mn_f_PopMenu(e){
	...
	mn_CurrentMenu.mouseX = mouse_x_position = (mn_IE) ? (e.clientX + document.body.scrollLeft) : e.pageX;
	mn_CurrentMenu.mouseY = mouse_y_position = (mn_IE) ? (e.clientY + document.body.scrollTop)  : e.pageY;
	...
	if(mn_IE5M && !mn_CurrentMenu.sizeFixed) mn_CurrentMenu.fixSize(false);
...
function mn_f_MenuOut() {
	if(mn_IE && event.srcElement.contains(event.toElement)) return;
    this.isOn = false;
    mn_UserOverMenu = false;
...
function mn_f_ShowChild(){
		if(!this.child) {
			...
			mn_CurrentItem = this;
			mn_CurrentMenu.itemElement = this;
			this.child = mn_f_MakeMenu(this.menu.count + "_"+(this.index+1));
			this.tree.treeParent = this.menu;
			this.tree.startChild = this.menu;
		}
		if(mn_IE5M && !this.child.sizeFixed) this.child.fixSize(false);

		if (this.tree.PositionUnder && (this.menu == this.tree.treeParent)) {
			this.child.xPos = parseInt(this.menu.style.left) + parseInt(this.style.left);
			this.child.yPos = parseInt(this.menu.style.top)  + this.menu.offsetHeight - (mn_BorderWidth);
		}
		else {
			if(this.ChildOverlap==null) {
				this.DistanceToRightEdge = parseInt(this.style.width);
				if(!mn_IEnoDTD) this.DistanceToRightEdge += (parseInt(this.style.paddingLeft)+parseInt(this.style.paddingRight)) + ((this.menu.isHorizontal && !this.isLastItem) ? mn_SeparatorSize : 0); 
			...
			if(mn_IE5M) {
				this.oL = parseInt(this.menu.style.left) - mn_ItemPadding;
				this.oL += this.offsetLeft;
				this.oT = parseInt(this.menu.style.top) - mn_ItemPadding;
				this.oT += this.offsetTop;
			}
			else {
				this.oL = (mn_IEpos) ? parseInt(this.menu.style.left) + mn_BorderWidth : 0;
				this.oL += this.offsetLeft;
				this.oT = (mn_IEpos) ? parseInt(this.menu.style.top) : -mn_BorderWidth;
				this.oT += this.offsetTop;
			}
...
function mn_f_ItemOver(){
	...
	if(mn_IE && event.srcElement.id == "mn_ImMore") return;
	if(this.hasRollover) {
...
function mn_f_ItemOut() {
	if (mn_IE && (event.srcElement.contains(event.toElement)
...
function mn_f_KeepInWindow() {
    var ExtraSpace     = 10;
	var WindowLeftEdge = (mn_IE) ? document.body.scrollLeft   : window.pageXOffset;
	var WindowTopEdge  = (mn_IE) ? document.body.scrollTop    : window.pageYOffset;
	var WindowWidth    = (mn_IE) ? document.body.clientWidth  : window.innerWidth;
	var WindowHeight   = (mn_IE) ? document.body.clientHeight : window.innerHeight;
