...
if ($_SESSION[logged]){
	...
	if($user_res && ! $user_res->EOF ){
		...
		if ($ctacte_r && !$ctacte_r->EOF){
			...
			$pdf->Cell(35,8,"Crédito","TB",0,'C');
			$pdf->Cell(30,8,"Débito","TB",0,'C');
			...
			while(!$ctacte_r->EOF){
				...
				if ($ctacte_r->fields['credito'] > 0){
					$credito = sprintf('%01.2f',$ctacte_r->fields['credito']);
				} else {
					$credito = "";
				}
				if ($ctacte_r->fields['debito'] > 0){
					$debito = sprintf('%01.2f',$ctacte_r->fields['debito']);
				} else {
					$debito = "";
				}
				...
				$pdf->Cell(35,6,$credito,0,0,'R');
				$pdf->Cell(30,6,$debito,0,0,'R');
			...
			$pdf->Output("I","Resumen de Cuenta.pdf");
