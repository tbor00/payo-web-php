...
if($user_res && ! $user_res->EOF ){
	...
	$query = "SELECT COUNT(id) FROM e_ctasctes WHERE cliente_eb=$cliente_eb AND gestion='".$gestion."'";
	...
	$from = ($pag-1) * $qlineas;
	if ($pag > 1){
		$query = "SELECT sum(debito)-sum(credito) as saldo FROM (select debito,credito from e_ctasctes WHERE cliente_eb=$cliente_eb AND gestion='".$gestion."' ORDER by fecha,id limit 0,$from) as rowtt";
		if ($ctacte_r=$db->Execute($query)){
			$saldo_ctacte = $ctacte_r->fields['saldo'];
		}	
	}
	$query = "SELECT * FROM e_ctasctes WHERE cliente_eb=$cliente_eb AND gestion='".$gestion."' ORDER by fecha,id limit $from, $qlineas";
	if ($ctacte_r=$db->Execute($query)){
		echo "<div class=\"row\">";
...
<?php			
			...
			if ($tipo_pago==1){
				echo "<A CLASS=\"payment btn btn-default\" HREF=\"auxiliar.php?op=cc_payment\" data-toggle=\"tooltip\" title=\"Pago\"><i class=\"fa fa-money\"></i></A>";
			}
			echo "<A HREF=\"auxiliar.php?op=cprint\" TARGET=\"_print\"  class=\"btn btn-default\" data-toggle=\"tooltip\" title=\"Versi&oacute;n para Imprimir\"><i class=\"fa fa-print\"></i></A>";
			echo "</div>";
		} else {
		...
		echo "<thead>\n";
		echo "<TR>\n";
		echo Titulo_Browse("Fecha","id_cotiza",$orden,$ord,"$PHP_SELF?op=$op&sop=$sop&pag=$pag$url_busqueda",0,0,"center");
		echo Titulo_Browse("Comprobante","fecha",$orden,$ord,"$PHP_SELF?op=$op&sop=$sop&pag=$pag$url_busqueda",0,0);
		...
		echo Titulo_Browse("Cr&eacute;dito","credito",$orden,$ord,"$PHP_SELF?op=$op&sop=$sop&pag=$pag$url_busqueda",0,0,"center");
		echo Titulo_Browse("D&eacute;bito","debito",$orden,$ord,"$PHP_SELF?op=$op&sop=$sop&pag=$pag$url_busqueda",0,0,"center");
		echo Titulo_Browse("Saldo","saldo",$orden,$ord,"$PHP_SELF?op=$op&sop=$sop&pag=$pag$url_busqueda",0,0,"center");
		echo "</TR>\n";
		echo "</thead>\n";
		...
		while (!$ctacte_r->EOF){
			$saldo_ctacte = $saldo_ctacte + $ctacte_r->fields['debito'] - $ctacte_r->fields['credito'];;
			echo "<TR>\n";
			echo "<TD class=\"text-right\">".timesql2std($ctacte_r->fields['fecha'])."</td>\n";
			echo "<TD class=\"text-left\">".$ctacte_r->fields['comprobante'].'&nbsp;'.$ctacte_r->fields['numero'];
			if (file_exists("facturas/".$ctacte_r->fields['comprobante'].$ctacte_r->fields['numero'].".pdf")){
				echo "&nbsp;<a href=\""."facturas/".$ctacte_r->fields['comprobante'].$ctacte_r->fields['numero'].".pdf\" target=\"".$ctacte_r->fields['comprobante'].$ctacte_r->fields['numero']."\">";
				echo "<i class=\"fa fa-file-pdf-o\" style=\"color:red;\"></i></a></TD>\n";
			}
			echo "</TD>\n";
			...
			echo "<TD class=\"text-right\">";
			if ($ctacte_r->fields['credito'] > 0){
				echo sprintf('%01.2f',$ctacte_r->fields['credito']);
			} else {
				echo "";
			}
			echo "</TD>\n";		
			echo "<TD class=\"text-right\">";
			if ($ctacte_r->fields['debito'] > 0){
				echo sprintf('%01.2f',$ctacte_r->fields['debito']);
			} else {
				echo "";
			}
			echo "</TD>\n";		
